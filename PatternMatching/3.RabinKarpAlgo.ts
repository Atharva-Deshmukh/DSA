...
basically (pat_len - 1 - i)

do the same for first window "aba"

---------------------------------------------------------------------------------
                        How to slide hash for next window
---------------------------------------------------------------------------------

Using rolling hash

Window - 1 => "aba" = (ASCII(a) * (base ^ 2) + ASCII(b) * (base ^ 1) + ASCII(a) * (base ^ 0))
Window - 2 => "bab" = (ASCII(b) * (base ^ 2) + ASCII(a) * (base ^ 1) + ASCII(b) * (base ^ 0))
...
Shift remaining hash to left by base
    ...
    and if we remove 1 => 023
    to shift 023 => 230, we do (n_remaining * base)

and add the hash of b now which is (i + pat_len) character

---------------------------------------------------------------------------------
                        How to choose BASE => 256
---------------------------------------------------------------------------------

Why 256?
Extended ASCII Support so that algo can include and handle all the characters
...
---------------------------------------------------------------------------------
                        How to choose a prime no. for mod => 1e9 + 7
---------------------------------------------------------------------------------
...
function rabinKarpAlgo(pattern: string, txt: string): number[] {
    ...
    patternHash = 0 initially
    patternHash = (base × patternHash + ASCII(a)) % mod  
    patternHash = (base × patternHash + ASCII(b)) % mod  
    patternHash = (base × patternHash + ASCII(c)) % mod  

    Both methods produce the same result, but the optimized method is computationally efficient.*/
    
    /* First window + hash for pattern string in one go 
